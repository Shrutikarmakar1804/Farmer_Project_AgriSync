...
// Demo trend data for the visual dashboard.
...
export default function History() {
  const totalValue = '₹300,072'
  const totalQuantity = '93.5 q'

  const maxMonthly = Math.max(...monthlyData.map(item => item.value))
  const maxYearly = Math.max(...yearlyData.map(item => item.value))

  const handleExport = () => {
    alert('Transaction history export is a frontend demo. Connect this button to your backend/export service later.')
  }
